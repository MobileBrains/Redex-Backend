# Take from https://gist.github.com/oivoodoo/5089237
namespace :grape do
  ...
  task :routes => :environment do
    ...
    params_str = ' params:'
    desc_limit = 45
    route_info = API::V1::API.routes.map {|r| [r, r.instance_variable_get(:@options)] }
    ...
    max_path_size     = route_info.map{|_,info| info[:namespace].sub(':version', info[:version]).size }.max
    ...
    route_info.each do |_,info|
      fields = [
        ...
        info[:method],
        mapped_prefix,
        info[:namespace].sub(':version', info[:version]),
